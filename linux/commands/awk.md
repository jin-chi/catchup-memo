...
- [応用](#応用)
  ...
  - [2つのファイルを特定のキーで結合する](#2つのファイルを特定のキーで結合する)
    - [ファイルマージ処理コマンド比較（join vs awk）](#ファイルマージ処理コマンド比較join-vs-awk)
...
## 2つのファイルを特定のキーで結合する
- 2つのファイルの特定のキーでマージする（両方にキーが存在する行だけを出力）
```bash
awk 'NR==FNR { a[$1]=$0; next } $1 in a { print a[$1], $2 }' input1.txt input2.txt
```
- 2つのファイルの特定のキーでマージする（1つ目のファイルにキーがなくても2つ目のファイルは全て出力する）
```bash
awk 'NR==FNR { a[$1]=$0; next } { print a[$1], $2 }' input1.txt input2.txt
```

### ファイルマージ処理コマンド比較（join vs awk）
| 項目 | join | awk |
| ---- | ---- | ---- |
| ソートの必要性 | 必要（必須条件） | 不要 |
| 処理速度 | 高速（ソート済みのデータに対して最適化されている） | join よりは遅い場合が多い |
| 複雑な処理 | 単純な結合のみ | マージと同時に複雑なデータ操作が可能 |
| メモリ使用量 | 少ない（行ごとに処理） | 多い（片方のファイルをメモリに読み込むため） |
| 使いやすさ | シンプルな結合なら簡単 | 少し複雑なスクリプト記述が必要 |
| | | |

- 結論
  ```
  まずは join でできないかを検討する。
  複雑な処理が必要で、 join ではできない場合は awk を検討する。
  ```